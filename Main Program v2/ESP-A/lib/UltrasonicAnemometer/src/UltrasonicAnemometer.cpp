...
UltrasonicAnemometer::UltrasonicAnemometer(uint8_t roPin, uint8_t diPin, uint8_t dePin, uint8_t rePin, uint8_t max485Pin)
    : ultrasonicAnemometer(roPin, diPin, false), roPin(roPin), diPin(diPin), dePin(dePin), rePin(rePin), max485Pin(max485Pin)
{
    totalWindSpeed_ms = 0;
